...
// @desc Get club events (only for club members)
// @route GET /api/student/clubs/:clubId/events
export const getClubEvents = async (req, res, next) => {
  try {
    ...
    const isMember = club.members.some(
      m => m.student.toString() === req.user._id.toString() && m.status === 'approved'
    );

    if (!isMember) {
      return next({ status: 403, message: "You must be an approved member to view events" });
    }
...
// @desc Get club announcements (only for club members)
// @route GET /api/student/clubs/:clubId/announcements
export const getClubAnnouncements = async (req, res, next) => {
  try {
    ...
    if (!isMember) {
      return next({ status: 403, message: "You must be an approved member to view announcements" });
    }
